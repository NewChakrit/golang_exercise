...
func workerProcess(in <-chan string) {
	...
	thisID := workerID
	for {
		fmt.Sprintf("%d: watting for input...\n", thisID)
		input := <-in
...
Is this fan out?
Ans: Yes
Are we "fanning out" work? Yes. We've launched several goroutines that are simultaneously publishing a
message onto our channel. The golang blog says, "Fan out means you have multiple functions reading from the"
same channel. So, okay, we're fanning out.

CHALLENGE #2
Is this fan on?
Ans: No
What is being "fanning in" here? We have launched several goroutines of the same function: workerProcess.
...
funcs could be processing in paraller: pulling values off the channel and processing them, There is no
